...
_______________
	...
	Since a text file is just a string of 8-bit ASCII characters, text can be encoded in an image the same way an image is encoded in an image. OIIO causes some data loss here, but words will still be readable. The program also saves the length of the text file in the last few bits of the image so that only the desired string will be extracted.
